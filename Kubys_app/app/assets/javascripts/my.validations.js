...
(function($) {
  $.fn.validate = function() {
    return this.filter('form[data-validate]').each(function() {
      ...
      form.find('[data-validate]:input:not(:radio,.masked)')
        .live('focusout',                function()          { $(this).isValid(settings.validators); });
      form.find('[data-validate]:input.masked:not(:radio)')
        .live('focusout:after:unmask',   function()          { $(this).isValid(settings.validators); });
      ...
      numericality: function(element, options) {
        ...
        for (var check in CHECKS) {
          if (options[check] != undefined) {
            if(element.val() != '' && !(new Function("return " + element.val() + CHECKS[check] + options[check])()))
